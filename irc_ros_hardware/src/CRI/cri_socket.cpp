...
: sock(0),
  ...
  maxUnprocessedMessages(100),  // Small for testing
  listCheckWaitMs(500),
  fragmentBuffer{0},
  fragmentLength(0)
{
...
void CriSocket::SeparateMessages(const char * msg)
{
  ...
  while (true) {
...

    {
      ...
      // RCLCPP_INFO(rclcpp::get_logger("iRC_CRI"), "%s", unprocessedMessages.front().c_str());
...
void CriSocket::ListCheckThreadFunction()
{
  ...
  while (continueReceive) {
    if (unprocessedMessages.size() > maxUnprocessedMessages) {
      ...
      while (unprocessedMessages.size() > (maxUnprocessedMessages)) {
...
bool CriSocket::IsConnected()
{
    return !connectionNeeded;
}


void CriSocket::SendUsePositionInterface(int cmdNr, bool enabled)
{
  std::ostringstream msg;
  msg << "CRISTART "
      << cmdNr
      << " CMD UsePositionInterface ";
  ...
  {
      SendMessage(msg.str());
      RCLCPP_INFO(rclcpp::get_logger("iRC_Ros"), "%s ", msg.str().c_str());
...
void CriSocket::SendConfigurePositionInterface(int cmdNr, bool enabled)
{   
    std::ostringstream msg;
    msg << "CRISTART "
        << cmdNr
        << " CONFIG SetPositionInterface ";
...
void CriSocket::RequestGetPositionInterface(int cmdNr)
{
  std::ostringstream msg;
  msg << "CRISTART "
      << cmdNr
      << " CMD GetPositionInterface "
      << "CRIEND";
