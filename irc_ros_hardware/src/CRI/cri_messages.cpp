...
MessageType CriMessage::GetMessageType(const std::string & msg)
{
  ...
  static const std::map<std::string, MessageType> message_type_map = {
    ...
    {cri_keywords::TYPE_EXECPAUSE, MessageType::EXECPAUSE},
    // {cri_keywords::POSITION, MessageType::POSITION},
...
Status::Status(const std::string & messageString, int criVersion) : CriMessage(MessageType::STATUS)
{
  ...
  case 16:
      ...
      break;
  case 17:
      //// DIn & Dout send as hexadecimal number
...
//Position::Position() : CriMessage(MessageType::POSITION)
//{}
//
//Position::Position(const std::string & posMsg) : CriMessage(MessageType::POSITION)
//{
//    // RCLCPP_INFO(rclcpp::get_logger("iRC_ROS::POSITION"), "%s", posMsg.c_str());
//    std::string::size_type posStart = posMsg.find("Pos");
//
//    if(posStart != std::string::npos)
//    {
//        std::string::size_type iJointsStart = posMsg.find("J");
//        std::string::size_type eJointsStart = posMsg.find("E");
//        std::string::size_type cartesianStart = posMsg.find("C");
//
//        if(!((posStart < iJointsStart)  && (iJointsStart < eJointsStart) && (eJointsStart < cartesianStart)))
//        {   
//            //NACHRICHT NICHT RICHTIG FORMATIERT
//            return;
//        }
//
//        std::string iJointsString = ParseMessageString(posMsg, iJointsStart, eJointsStart, 1);
//        std::string eJointsString = ParseMessageString(posMsg, eJointsStart, cartesianStart, 1);
//        std::string cartesianString = ParseMessageString(posMsg, cartesianStart, posMsg.size() + 1, 1);
//
//
//        std::array<float, 6> internalAxes;
//        std::array<float, 3> externalAxes; 
//
//        // RCLCPP_INFO(rclcpp::get_logger("iRC_ROS::POSITIONMESSAGE"), "\n Full: %s \n Joints: %s \n Cartesian: %s \n External: %s", posMsg.c_str(),iJointsString.c_str(), cartesianString.c_str(), eJointsString.c_str());
//
//        FillArray(internalAxes, iJointsString);
//        FillArray(m_cartesianPose, cartesianString);
//        FillArray(externalAxes, eJointsString);
//        
//
//        for (int i=0; i<internalAxes.size(); i++)
//        {
//            m_Joints[i] = internalAxes[i];
//        }
//
//        for (int i=0; i<externalAxes.size(); i++)
//        {
//            m_Joints[internalAxes.size() + i] = externalAxes[i];
//        }
//    }    
//}
